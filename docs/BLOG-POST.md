---
title: "GroundedRelay: shopping with an agent without surrendering the checkout"
status: draft-for-final-runtime-and-name-review
---

# GroundedRelay: shopping with an agent without surrendering the checkout

> Editorial gate: do not publish this article until the GroundedRelay-owned fictional
...
> Naming note: exact web, npm, PyPI, GitHub repository-name, and
> `.com`/`.dev`/`.app`/`.io` DNS screens on 2026-08-26 found no direct
> GroundedRelay software or product match. This was only a preliminary collision
> screen, not legal advice or trademark clearance.
...
We built **GroundedRelay** around a different idea: the agent and the person should
work on the same visible page, but the person should retain the consequential
decision.

GroundedRelay uses [WebMCP](https://developers.openai.com/codex/webmcp) to expose
...
reveals only a GroundedRelay-owned fictional link on `groundedrelay-merchant.pages.dev`; it
...
GroundedRelay's release architecture is a reusable cross-origin pattern across three
...
a parsed object. GroundedRelay re-serializes the proxy input rather than assuming the
...
catalogue. GroundedRelay takes that freedom away deliberately.
...
GroundedRelay treats those as product requirements:
...
- It carries exact variant IDs and options through inspection and basket
  mutation. The public build uses stable GroundedRelay-owned demo IDs.
...
GroundedRelay has no application backend, proxy, analytics service, model-key store,
or shared search index. The release is configured so public HTTPS loads a
GroundedRelay-owned static fixture and ranks it in the shopper's browser. It does not
...
are data, not instructions for the agent. GroundedRelay uses bounded schemas and
compact typed outputs rather than dropping entire webpages into a model's
context.

## The last action belongs to the person

The most important GroundedRelay action does not buy anything. It prepares a handoff
...
GroundedRelay-owned merchant host at `groundedrelay-merchant.pages.dev`. It never opens
...
The submission candidate uses three GroundedRelay-owned fictional African catalogues:
...
its handoff links stay on GroundedRelay's owned merchant-demo origin.
...
GroundedRelay is not claiming to be a new pan-African marketplace. It is showing how
...
GroundedRelay tree. Direct `executeTool()` calls show that
...
GroundedRelay's promise is simple:
